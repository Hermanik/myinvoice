  ...
  >
    ...
    <div v-else>
      ...
      <div v-if="selCount > 0" class="flex items-center gap-2 mb-3 px-3 py-2 bg-primary-50 border border-primary-200 rounded-lg text-sm flex-wrap">
        ...
        <button v-if="auth.canWrite" type="button" class="cursor-pointer inline-flex items-center gap-1.5 h-8 px-2.5 text-sm font-medium rounded-md border border-neutral-300 bg-surface text-neutral-700 hover:bg-neutral-50" @click="bulkTag">
      ...
      <div v-else>
        ...
        <div v-if="folders.length" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 mb-4">
          <div
            v-for="f in folders"
            :key="f.id"
            class="group flex items-center gap-2 px-3 py-2.5 bg-surface border border-neutral-200 rounded-lg hover:border-primary-300 cursor-pointer"
